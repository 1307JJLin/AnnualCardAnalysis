...
def variance_std_ratio(x):
    y = np.var(x)
    return y/np.sqrt(y) if y != 0 else np.nan
...
def drawdown_duration(series):
    ...
    i = np.argmax(np.maximum.accumulate(series) - series)
    j = k if len(series[:i]) == 0 else np.argmax(series[:i])
    return k-j
...
def count_above(x, t):
    return np.nan if len(x)==0 else np.sum(x >= t) / len(x)
...
def quantile(x, q):
    return np.NaN if len(x) == 0 else np.quantile(x, q)
